...
while True:
    ...
    for i in range(len(boxes)):
        if i in indexes:
            ...
            # Exibir o tempo de exposição
            person_id = i + 1
            if person_id in detection_times:
                total_exposure = detection_times[person_id]['total_exposure']
                cv2.putText(frame, f"Exposure: {total_exposure:.1f}s", (x, y + h + 40), font, 1, green_color, 1)
...
df = pd.DataFrame.from_dict(detection_times, orient='index')

# Salvando o DataFrame em um arquivo CSV
df.to_csv('detected_people_data.csv', index_label='person_id')
